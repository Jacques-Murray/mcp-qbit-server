...
class ToolService {
  ...
  async callTool(params) {
    ...
    if (!paramsValidation.success) {
      console.warn('[ToolService] Invalid tools/call parameters:', paramsValidation.error.format());
    ...
    if (!tool) {
      console.warn(`[ToolService] Tool not found: ${name}`);
    ...
    try {
      ...
      console.log(`[ToolService] Tool '${name}' executed successfully.`);
      return {
        isError: false,
        content,
      };
