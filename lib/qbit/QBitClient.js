...
class QBitClient {
  ...
  async #makeRequest(requestFn) {
    ...
    try {
      ...
      if (error.code === 'ETIMEDOUT' || error.code === 'ECONNABORTED') {
        throw new Error('Request to qBittorrent timed out. The service may be slow to respond, or the timeout setting may need to be increased.');
      }
  ...
  async addTorrent(url) {
    ...
    return data && typeof data === 'string' && data.trim() === 'Ok.';
  }
}
